...
eButtonsState LiquidCrystal_I2C_Menu_Btns::getButtonsState()
{
  ...
  if (millis() - _prevPoolTime > BUTTONS_POOL_DELAY)
  {
    ...
    if (newStates != _prevButtonStates)
      _pressTime = millis();
...
bool LiquidCrystal_I2C_Menu_Btns::isEditable(const char *ch, const char availSymbols[])
{
  if (ch[0] == '\0')
    return 1;
